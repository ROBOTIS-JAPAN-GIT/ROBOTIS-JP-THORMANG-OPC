...
#include "../include/jp_thormang3_demo/qnode.hpp"
...
QNodeThor3::QNodeThor3(int argc, char** argv)
    : init_argc_(argc),
      init_argv_(argv),
      marker_name_("jp_thormang3_demo_marker"),
...
{
  ros::init(init_argc_, init_argv_, "jp_thormang3_demo");
  ...
  package_name_ = ros::package::getPath("jp_thormang3_demo");
  ...
  // Manipulation
  kenematics_pose_sub_ = nh.subscribe("/jp_thormang3_demo/ik_target_pose", 10, &QNodeThor3::getKinematicsPoseCallback,
                                      this);
  ...
  // Config
  std::string default_config_path = ros::package::getPath("jp_thormang3_demo") + "/config/demo_config.yaml";
  std::string config_path = nh.param<std::string>("demo_config", default_config_path);
  parseJointNameFromYaml(config_path);

  std::string motion_path = ros::package::getPath("jp_thormang3_demo") + "/config/motion.yaml";
...
// Get balance_yaml_path
std::string QNodeThor3::getBalanceYAMLPath()
{
  return balance_yaml_path_;
}
