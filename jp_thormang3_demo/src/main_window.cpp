...
#include <QMessageBox>
#include <iostream>
#include "../include/jp_thormang3_demo/main_window.hpp"

/*****************************************************************************
 ** Namespaces
 *****************************************************************************/

namespace jp_thormang3_demo
{
...
void MainWindow::on_button_param_load_clicked(bool check)
{
  // std::string balance_yaml_path =
  YAML::Node doc;
  try
  {
    doc = YAML::LoadFile( qnode_thor3_.getBalanceYAMLPath().c_str() );
    ...
    ui_.A1_spinbox_cob_x_offset->setValue(cob_x_offset_m);
    ui_.B1_spinbox_cob_y_offset->setValue(cob_y_offset_m);
    ui_.C1_spinbox_hip_roll_swap_angle->setValue(hip_roll_swap_angle_rad);
    ui_.D1_spinbox_foot_roll_gyro_p_gain->setValue(foot_roll_gyro_p_gain);
    ui_.E1_spinbox_foot_roll_gyro_d_gain->setValue(foot_roll_gyro_d_gain);
    ui_.F1_spinbox_foot_pitch_gyro_p_gain->setValue(foot_pitch_gyro_p_gain);
    ui_.G1_spinbox_foot_pitch_gyro_d_gain->setValue(foot_pitch_gyro_d_gain);
    ui_.H1_spinbox_foot_roll_angle_p_gain->setValue(foot_roll_angle_p_gain);
    ui_.I1_spinbox_foot_roll_angle_d_gain->setValue(foot_roll_angle_d_gain);
    ui_.J1_spinbox_foot_pitch_angle_p_gain->setValue(foot_pitch_angle_p_gain);
    ui_.K1_spinbox_foot_pitch_angle_d_gain->setValue(foot_pitch_angle_d_gain);
    ui_.L1_spinbox_roll_gyro_cut_off_frequency->setValue(roll_gyro_cut_off_frequency);
    ui_.M1_spinbox_pitch_gyro_cut_off_frequency->setValue(pitch_gyro_cut_off_frequency);
    ui_.N1_spinbox_roll_angle_cut_off_frequency->setValue(roll_angle_cut_off_frequency);
    ui_.O1_spinbox_pitch_angle_cut_off_frequency->setValue(pitch_angle_cut_off_frequency);
    ui_.A3_spinbox_foot_x_force_p_gain->setValue(foot_x_force_p_gain);
    ui_.B3_spinbox_foot_x_force_d_gain->setValue(foot_x_force_d_gain);
    ui_.C3_spinbox_foot_y_force_p_gain->setValue(foot_y_force_p_gain);
    ui_.D3_spinbox_foot_y_force_d_gain->setValue(foot_y_force_d_gain);
    ui_.E3_spinbox_foot_z_force_p_gain->setValue(foot_z_force_p_gain);
    ui_.F3_spinbox_foot_z_force_d_gain->setValue(foot_z_force_d_gain);
    ui_.G3_spinbox_foot_roll_torque_p_gain->setValue(foot_roll_torque_p_gain);
    ui_.H3_spinbox_foot_roll_torque_d_gain->setValue(foot_roll_torque_d_gain);
    ui_.I3_spinbox_foot_pitch_torque_p_gain->setValue(foot_pitch_torque_p_gain);
    ui_.J3_spinbox_foot_pitch_torque_d_gain->setValue(foot_pitch_torque_d_gain);
    ui_.K3_spinbox_foot_x_force_cut_off_frequency->setValue(foot_x_force_cut_off_frequency);
    ui_.L3_spinbox_foot_y_force_cut_off_frequency->setValue(foot_y_force_cut_off_frequency);
    ui_.M3_spinbox_foot_z_force_cut_off_frequency->setValue(foot_z_force_cut_off_frequency);
    ui_.N3_spinbox_foot_roll_torque_cut_off_frequency->setValue(foot_roll_torque_cut_off_frequency);
    ui_.O3_spinbox_foot_pitch_torque_cut_off_frequency->setValue(foot_pitch_torque_cut_off_frequency);
...
void MainWindow::on_button_param_save_clicked(bool check)
{
  ...
  YAML::Emitter out; // TODO: add comment and return
  out << YAML::BeginMap;
  out << YAML::Key << "cob_x_offset_m" << YAML::Value << ui_.A1_spinbox_cob_x_offset->value();
  out << YAML::Key << "cob_y_offset_m" << YAML::Value << ui_.B1_spinbox_cob_y_offset->value();
  out << YAML::Key << "hip_roll_swap_angle_rad" << YAML::Value << ui_.C1_spinbox_hip_roll_swap_angle->value();
  out << YAML::Key << "foot_roll_gyro_p_gain" << YAML::Value << ui_.D1_spinbox_foot_roll_gyro_p_gain->value();
  out << YAML::Key << "foot_roll_gyro_d_gain" << YAML::Value << ui_.E1_spinbox_foot_roll_gyro_d_gain->value();
  out << YAML::Key << "foot_pitch_gyro_p_gain" << YAML::Value << ui_.F1_spinbox_foot_pitch_gyro_p_gain->value();
  out << YAML::Key << "foot_pitch_gyro_d_gain" << YAML::Value << ui_.G1_spinbox_foot_pitch_gyro_d_gain->value();
  out << YAML::Key << "foot_roll_angle_p_gain" << YAML::Value << ui_.H1_spinbox_foot_roll_angle_p_gain->value();
  out << YAML::Key << "foot_roll_angle_d_gain" << YAML::Value << ui_.I1_spinbox_foot_roll_angle_d_gain->value();
  out << YAML::Key << "foot_pitch_angle_p_gain" << YAML::Value << ui_.J1_spinbox_foot_pitch_angle_p_gain->value();
  out << YAML::Key << "foot_pitch_angle_d_gain" << YAML::Value << ui_.K1_spinbox_foot_pitch_angle_d_gain->value();
  out << YAML::Key << "roll_gyro_cut_off_frequency" << YAML::Value << ui_.L1_spinbox_roll_gyro_cut_off_frequency->value();
  out << YAML::Key << "pitch_gyro_cut_off_frequency" << YAML::Value << ui_.M1_spinbox_pitch_gyro_cut_off_frequency->value();
  out << YAML::Key << "roll_angle_cut_off_frequency" << YAML::Value << ui_.N1_spinbox_roll_angle_cut_off_frequency->value();
  out << YAML::Key << "pitch_angle_cut_off_frequency" << YAML::Value << ui_.O1_spinbox_pitch_angle_cut_off_frequency->value();
  out << YAML::Key << "foot_x_force_p_gain" << YAML::Value << ui_.A3_spinbox_foot_x_force_p_gain->value();
  out << YAML::Key << "foot_x_force_d_gain" << YAML::Value << ui_.B3_spinbox_foot_x_force_d_gain->value();
  out << YAML::Key << "foot_y_force_p_gain" << YAML::Value << ui_.C3_spinbox_foot_y_force_p_gain->value();
  out << YAML::Key << "foot_y_force_d_gain" << YAML::Value << ui_.D3_spinbox_foot_y_force_d_gain->value();
  out << YAML::Key << "foot_z_force_p_gain" << YAML::Value << ui_.E3_spinbox_foot_z_force_p_gain->value();
  out << YAML::Key << "foot_z_force_d_gain" << YAML::Value << ui_.F3_spinbox_foot_z_force_d_gain->value();
  out << YAML::Key << "foot_roll_torque_p_gain" << YAML::Value << ui_.G3_spinbox_foot_roll_torque_p_gain->value();
  out << YAML::Key << "foot_roll_torque_d_gain" << YAML::Value << ui_.H3_spinbox_foot_roll_torque_d_gain->value();
  out << YAML::Key << "foot_pitch_torque_p_gain" << YAML::Value << ui_.I3_spinbox_foot_pitch_torque_p_gain->value();
  out << YAML::Key << "foot_pitch_torque_d_gain" << YAML::Value << ui_.J3_spinbox_foot_pitch_torque_d_gain->value();
  out << YAML::Key << "foot_x_force_cut_off_frequency" << YAML::Value << ui_.K3_spinbox_foot_x_force_cut_off_frequency->value();
  out << YAML::Key << "foot_y_force_cut_off_frequency" << YAML::Value << ui_.L3_spinbox_foot_y_force_cut_off_frequency->value();
  out << YAML::Key << "foot_z_force_cut_off_frequency" << YAML::Value << ui_.M3_spinbox_foot_z_force_cut_off_frequency->value();
  out << YAML::Key << "foot_roll_torque_cut_off_frequency" << YAML::Value << ui_.N3_spinbox_foot_roll_torque_cut_off_frequency->value();
  out << YAML::Key << "foot_pitch_torque_cut_off_frequency" << YAML::Value << ui_.O3_spinbox_foot_pitch_torque_cut_off_frequency->value();

  out << "";
  out << YAML::EndMap;

  std::ofstream fout(qnode_thor3_.getBalanceYAMLPath().c_str());
...
{
  QSettings settings("Qt-Ros Package", "jp_thormang3_demo");
